...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (($_POST['action'] ?? '') === 'react') {
        $reactionController->react($id, (string) ($_POST['tipo'] ?? 'like'));
...
$alreadyReacted = $reactionController->alreadyReacted($id);
        ...
        <div class="reaction-group">
            <?php foreach (REACTION_TYPES as $tipo => $info): ?>
                <form method="post" action="<?= BASE_URL ?>/views/public/news-detail.php?id=<?= $id ?>">
                    ...
                    <button type="submit" class="btn reaction-btn" <?= $alreadyReacted ? 'disabled' : '' ?> title="<?= e($info['label']) ?>">
